              ...
              {pages.map((page) => (
                <TableRow key={page.id} className="border-gray-800 hover:bg-gray-700/50">
                  ...
                  <TableCell>
                    <Badge 
                      className={
                        page.status === 'published' ? 'bg-green-600 hover:bg-green-600/90 text-white' :
                        page.status === 'scheduled' ? 'bg-blue-600 hover:bg-blue-600/90 text-white' :
                        'bg-gray-600 hover:bg-gray-600/90 text-white'
                      }
                    >
                      {page.status === 'draft' ? 'Brouillon' : page.status === 'published' ? 'Publiée' : 'Programmée'}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-gray-400">{new Date(page.created_at).toLocaleDateString()}</TableCell>
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <Button variant="outline" size="icon" className="border-gray-600 text-gray-300 hover:bg-gray-700 hover:text-white" disabled={page.status !== 'published'}>
